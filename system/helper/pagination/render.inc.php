...
<?php
        ...
        $first = $get;
        $first[$key] = 1;
        $firsthref = ($target) . '?' . http_build_query($first);
        $firsthref = preg_replace(
            array('/=$/', '/=&/'),
            array('', '&'),
            $firsthref
        );
...
?>
    <li class="<?= implode(' ', $classes) ?>"><a href="<?= ($firsthref) ?>">First</a></li>
...
<?php
        ...
        $last = $get;
        $last[$key] = $pages;
        $lasthref = ($target) . '?' . http_build_query($last);
        $lasthref = preg_replace(
            array('/=$/', '/=&/'),
            array('', '&'),
            $lasthref
        );
...
?>
    ...
    <li class="<?= implode(' ', $classes) ?>"><a href="<?= ($lasthref) ?>">Last</a></li>
</ul>
<?php
    }