				...
				<div class="panel-body">
					...
					if(Io::param('update')){
						if($category->update()){
							...
							header('Location: '.BASE.BACKEND.'/index.php?menu=Event&sub=category&action=edit&id=' . Io::param('id'));
						}
					}
					
					$language = $ARM->language->retrieve();

					$edit = $category->getById(Io::param('id'));
					?>
						...
						<select name="parent" class="form-control input-sm show-tick" data-live-search="true" required>
							...
							function buildMenu($data, $controller, $dash)
							{
								...
								foreach ($data as $k => $v) {
									...
									echo "<option value=" . $v->id . ( $edit->parent == $v->id ? ' selected' : '' ) . ">" . $strip . $controller->Detail->getTitle($v->id, 1) . "</option>";
		                ...
		                <?php foreach ($ARM->language->retrieve() as $key => $value) { ?>
		                  ...
		                  <div role="tabpanel" class="tab-pane <?php echo $key > 0 ? '' : 'active'?>" id="<?php echo $value->kode?>">
						  	...
						  	<div class="form-group">
						    	...
						    	<input name="title_<?php echo $value->kode?>" type="text" class="form-control input-sm" value="<?php echo $category->Detail->getTitle($edit->id, $value->id)?>" required>
						  	</div>

						  	<div class="form-group">
								<label>Description :</label>
								<textarea name="description_<?php echo $value->kode?>" id="description_<?php echo $value->kode?>" class="form-control input-sm"><?php echo $category->Detail->getDescription($edit->id, $value->id)?></textarea>
							</div>
